...
class Book_Now_Calendar_Sync {
	 ...
	 *
	 * @var Book_Now_Google_Calendar|null
	 */
	private $google;
	 ...
	 *
	 * @var Book_Now_Microsoft_Calendar|null
	 */
	private $microsoft;
	...
	public function __construct() {
		...
		$integration_settings = class_exists( 'Book_Now_Encryption' )
			? Book_Now_Encryption::get_integration_settings()
			: array();

		$this->settings = array_merge( $calendar_settings, $integration_settings );
	...
	public function sync_booking_created( $booking_id ) {
		...
		// Sync to Google Calendar, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && $this->google->is_authenticated() ) {
		...
		// Sync to Microsoft Calendar, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && $this->microsoft->is_authenticated() ) {
	...
	public function sync_booking_confirmed( $booking_id ) {
		...
		// Create calendar events if they do not exist yet.
		// Sync to Google Calendar, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && $this->google->is_authenticated() && empty( $booking->google_event_id ) ) {
			$event_id = $this->google->create_event( $booking );
		...
		// Sync to Microsoft Calendar, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && $this->microsoft->is_authenticated() && empty( $booking->microsoft_event_id ) ) {
			$event_id = $this->microsoft->create_event( $booking );
	...
	public function sync_booking_updated( $booking_id ) {
		...
		// Update Google Calendar event, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && ! empty( $booking->google_event_id ) ) {
			$this->google->update_event( $booking->google_event_id, $booking );
		}

		// Update Microsoft Calendar event, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && ! empty( $booking->microsoft_event_id ) ) {
	...
	public function sync_booking_cancelled( $booking_id ) {
		...
		// Delete Google Calendar event, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && ! empty( $booking->google_event_id ) ) {
			$this->google->delete_event( $booking->google_event_id );
		}

		// Delete Microsoft Calendar event, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && ! empty( $booking->microsoft_event_id ) ) {
	...
	private function is_google_enabled() {
		return ! empty( $this->settings['google_sync_enabled'] ) || ! empty( $this->settings['google_calendar_enabled'] );
	}
	...
	private function is_microsoft_enabled() {
		return ! empty( $this->settings['microsoft_sync_enabled'] ) || ! empty( $this->settings['microsoft_calendar_enabled'] );
	}
	...
	public function is_time_available( $date, $time, $duration ) {
		...
		// Check Google Calendar, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && $this->google->is_authenticated() ) {
		...
		// If already unavailable, there is no need to check Microsoft.
		if ( false === $available ) {
			return false;
		}

		// Check Microsoft Calendar, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && $this->microsoft->is_authenticated() ) {
	...
	public function get_busy_times( $date_from, $date_to ) {
		...
		// Get Google Calendar busy times, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && $this->google->is_authenticated() ) {
		...
		// Get Microsoft Calendar busy times, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && $this->microsoft->is_authenticated() ) {
	...
	public function sync_all_pending( $limit = 50 ) {
		...
		$bookings = Book_Now_Booking::get_all(
			array(
				'status'  => 'confirmed',
				'limit'   => $limit,
				'orderby' => 'created_at',
		...
		foreach ( $bookings as $booking ) {
			...
			// Check if Microsoft sync is needed.
			if ( $this->is_microsoft_enabled() && null !== $this->microsoft && $this->microsoft->is_authenticated() && empty( $booking->microsoft_event_id ) ) {
				$needs_sync = true;
			}
	...
	/**
	 * Manually sync booking to calendars.
	 *
	 * @param int $booking_id Booking ID.
	...
	public function manual_sync( $booking_id ) {
		...
		// Sync to Google, with a null check.
		if ( $this->is_google_enabled() && null !== $this->google && $this->google->is_authenticated() ) {
		...
		// Sync to Microsoft, with a null check.
		if ( $this->is_microsoft_enabled() && null !== $this->microsoft && $this->microsoft->is_authenticated() ) {
