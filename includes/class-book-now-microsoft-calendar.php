<?php
/**
 * Book Now Microsoft Calendar file.
 *
 * @package BookNow
 */
...
class Book_Now_Microsoft_Calendar {
	...
	private function load_settings(): void {
		...
		$this->settings = array_merge(
			...
			array(
				...
				'microsoft_calendar_id'   => $integration_settings['microsoft_calendar_id'] ?? ( $calendar_settings['microsoft_calendar_id'] ?? 'primary' ),
			)
		);
	...
	public function process_oauth_callback(): void {
		...
		if ( ! isset( $_GET['page'] ) || 'book-now-settings' !== sanitize_text_field( wp_unslash( $_GET['page'] ) ) ) {
			return;
		}
		// Support both 'integration' and 'integrations' tab names for compatibility.
		$tab = isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] ) ) : '';
		if ( 'integration' !== $tab && 'integrations' !== $tab ) {
			return;
		}
		if ( ! isset( $_GET['microsoft_oauth'] ) || 'callback' !== sanitize_text_field( wp_unslash( $_GET['microsoft_oauth'] ) ) ) {
			return;
		}
	...
	public function handle_oauth_callback( string $code ) {
		...
		$tokens = array(
			...
			'microsoft_user_name'      => '',
			'microsoft_last_tested_at' => $this->settings['microsoft_last_tested_at'] ?? '',
		);
	 ...
	 *
	 * @return array{connected: bool, email: string, calendar_name: string, calendar_id: string, expires_at: string, error: string} Connection status details.
	 */
	public function get_connection_status(): array {
		$status = array(
			...
			'email'          => '',
			'calendar_name'  => '',
			'calendar_id'    => $this->settings['microsoft_calendar_id'] ?? 'primary',
			'last_tested_at' => '',
			'expires_at'     => '',
			'error'          => '',
		);
		...
		$status['calendar_name']  = $result['calendar_name'] ?? $this->settings['microsoft_user_name'] ?? __( 'Primary Calendar', 'book-now-kre8iv' );
		$status['calendar_id']    = $result['calendar_id'] ?? ( $this->settings['microsoft_calendar_id'] ?? 'primary' );
		$status['last_tested_at'] = $this->settings['microsoft_last_tested_at'] ?? '';
	...
	private function api_request( string $endpoint, string $method = 'GET', ?array $body = null ) {
		...
		if ( 'DELETE' === $method && 204 === $status_code ) {
	...
	public function create_event( object $booking ) {
		try {
			...
			// Use booking duration as fallback if type is not found.
			$duration  = $type ? $type->duration : ( $booking->duration ?? 60 );
			$type_name = $type ? $type->name : __( 'Consultation', 'book-now-kre8iv' );

			$timezone_setting = booknow_get_setting( 'general', 'timezone' );
			$timezone         = $timezone_setting ? $timezone_setting : 'UTC';
			...
			$result = $this->api_request( $this->get_events_endpoint(), 'POST', $event_data );
	...
	public function update_event( string $event_id, object $booking ) {
		try {
			...
			$result = $this->api_request( $this->get_event_endpoint( $event_id ), 'PATCH', $event_data );
	...
	public function delete_event( string $event_id ) {
		$result = $this->api_request( $this->get_event_endpoint( $event_id ), 'DELETE' );
	 ...
	 *
	 * @return array{success: bool, user_name: string, user_email: string, calendar_name: string, calendar_id: string}|\WP_Error Connection test result.
	 */
	public function test_connection() {
		...
		$calendar_id   = $this->settings['microsoft_calendar_id'] ?? 'primary';
		$calendar_name = __( 'Primary Calendar', 'book-now-kre8iv' );

		if ( 'primary' === $calendar_id ) {
			$calendar_result = $this->api_request( '/me/calendar' );
		} else {
			$calendar_result = $this->api_request( '/me/calendars/' . rawurlencode( $calendar_id ) );
		}

		if ( ! is_wp_error( $calendar_result ) ) {
			$calendar_name = $calendar_result['name'] ?? $calendar_name;
		}
	...
	/**
	 * Verify read/write access for the selected Microsoft calendar.
	 *
	 * @return array{success: bool, calendar_name: string, calendar_id: string, message: string}|\WP_Error
	 */
	public function test_selected_calendar_access() {
		if ( ! $this->is_authenticated() ) {
			return new WP_Error( 'not_authenticated', __( 'Microsoft Calendar not authenticated.', 'book-now-kre8iv' ) );
		}
		...
		$timezone         = $timezone_setting ? $timezone_setting : 'UTC';
		$tz_object        = new DateTimeZone( $timezone );
		$start_datetime   = new DateTime( 'now', $tz_object );
		$start_datetime->modify( '+10 minutes' );
		$end_datetime = clone $start_datetime;
		$end_datetime->modify( '+5 minutes' );

		$event_data = array(
			'subject' => __( 'Book Now Calendar Access Test', 'book-now-kre8iv' ),
			'body'    => array(
				'contentType' => 'text',
				'content'     => __( 'Temporary event created to verify Book Now calendar read/write access.', 'book-now-kre8iv' ),
			),
			'showAs'  => 'free',
			'start'   => array(
				'dateTime' => $start_datetime->format( 'Y-m-d\TH:i:s' ),
		...
		$create_result = $this->api_request( $this->get_events_endpoint(), 'POST', $event_data );
		...
		if ( ! empty( $create_result['id'] ) ) {
			$this->api_request( $this->get_event_endpoint( $create_result['id'] ), 'DELETE' );
		}

		$this->record_last_tested_at();

		return array(
			'success'       => true,
			'calendar_name' => $calendar_name,
			'calendar_id'   => $calendar_id,
			'message'       => __( 'Microsoft Calendar read/write access verified successfully.', 'book-now-kre8iv' ),
		);
	}
	...
	public function is_time_available( string $date, string $time, int $duration ) {
		...
		$endpoint = $this->get_calendar_view_endpoint( $start_str, $end_str );
		...
		// If any events are found, the time slot is busy.
	...
	/**
	 * Record last successful selected calendar test time.
	 *
	 * @return bool
	 */
	private function record_last_tested_at(): bool {
		$tokens = get_option( 'booknow_microsoft_tokens', array() );

		if ( ! empty( $tokens ) ) {
			$tokens = $this->decrypt_tokens( $tokens );
		}

		$tokens['microsoft_last_tested_at'] = current_time( 'c' );

		$this->settings['microsoft_last_tested_at'] = $tokens['microsoft_last_tested_at'];

		return $this->save_tokens( $tokens );
	}

	/**
	 * Get events endpoint for the selected Microsoft calendar.
	 *
	 * @return string
	 */
	private function get_events_endpoint(): string {
		$calendar_id = $this->settings['microsoft_calendar_id'] ?? 'primary';
		...
		return '/me/calendars/' . rawurlencode( $calendar_id ) . '/events';
	}

	/**
	 * Get event endpoint for the selected Microsoft calendar.
	 *
	 * @param string $event_id Event ID.
	 * @return string
	 */
	private function get_event_endpoint( string $event_id ): string {
		$calendar_id = $this->settings['microsoft_calendar_id'] ?? 'primary';

		if ( 'primary' === $calendar_id ) {
			return '/me/events/' . rawurlencode( $event_id );
		}

		return '/me/calendars/' . rawurlencode( $calendar_id ) . '/events/' . rawurlencode( $event_id );
	}

	/**
	 * Get calendar view endpoint for the selected Microsoft calendar.
	 *
	 * @param string $start_str Start datetime string.
	 * @param string $end_str   End datetime string.
	 * @return string
	 */
	private function get_calendar_view_endpoint( string $start_str, string $end_str ): string {
		$query       = '?startDateTime=' . rawurlencode( $start_str ) . '&endDateTime=' . rawurlencode( $end_str );
		$calendar_id = $this->settings['microsoft_calendar_id'] ?? 'primary';
		...
		return '/me/calendars/' . rawurlencode( $calendar_id ) . '/calendarView' . $query;
	}
}
