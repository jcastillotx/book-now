...
class Book_Now_Google_Calendar {
	...
	public function get_connection_status(): array {
		...
		if ( $status['connected'] ) {
			...
			$status['last_tested_at'] = $connection_meta['last_tested_at'] ?? '';

			// If we do not have metadata, try to fetch it.
	...
	private function update_connection_metadata(): bool {
		try {
			$existing = get_option( self::CONNECTION_META_KEY, array() );
			$metadata = array(
				'email'          => '',
				'calendar_name'  => '',
	...
	private function record_last_tested_at(): bool {
		$metadata                   = get_option( self::CONNECTION_META_KEY, array() );
		$metadata['last_tested_at'] = current_time( 'c' );
	...
	public function create_event( $booking ) {
		...
		try {
			...
			$timezone_string = booknow_get_setting( 'general', 'timezone' );
			$timezone_name   = $timezone_string ? $timezone_string : 'UTC';
	...
	/**
	 * Verify read/write access for the selected calendar.
	 *
	...
	public function test_selected_calendar_access() {
		...
		try {
			...
			$event = new Google_Service_Calendar_Event(
				array(
					'summary'      => __( 'Book Now Calendar Access Test', 'book-now-kre8iv' ),
					'description'  => __( 'Temporary event created to verify Book Now calendar read/write access.', 'book-now-kre8iv' ),
					'visibility'   => 'private',
			...
			$created_event = $this->service->events->insert( $calendar_id, $event );
			$this->service->events->delete( $calendar_id, $created_event->getId() );
			...
			return array(
				...
				'calendar_name' => $calendar->getSummary(),
				'message'       => __( 'Google Calendar read/write access verified successfully.', 'book-now-kre8iv' ),
			);
		} catch ( Exception $e ) {
			Book_Now_Logger::error( 'Google selected calendar access test failed', array( 'error' => $e->getMessage() ) );
			return new WP_Error( 'calendar_access_test_failed', $e->getMessage() );
		}
	}
	...
	public function is_time_available( string $date, string $time, int $duration ) {
		...
		try {
			$timezone_setting = booknow_get_setting( 'general', 'timezone' );
			$timezone_name    = $timezone_setting ? $timezone_setting : 'UTC';
			$timezone         = new DateTimeZone( $timezone_name );
			...
			return 0 === count( $events->getItems() );
